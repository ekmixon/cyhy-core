...
class TestStateManager:
    ...
    def test_was_failure(self, h, state_manager, stage, status):
        ...
        ), "status should have transitioned from to WAITING but did not."
    ...
    def test_ready_waiting_to_running(self, h, state_manager, stage, status):
        ...
        ), "status should have transitioned from READY to RUNNING but did not."

    @pytest.mark.parametrize(
        ("i_stage,up,f_stage,f_status"),
        [
    ...
    def test_running_open_ports(
        self, h, state_manager, i_stage, has_open_ports, f_stage, f_status
    ):
        h["stage"] = i_stage
        h["status"] = STATUS.RUNNING
