...
class CHDatabase(object):
    ...
    SNAPSHOT_CLOSED_TICKET_HISTORY_DAYS = (
        365  # number of days of closed tickets to include in closed tix metrics
    )
    ...
    def transition_host(
        ...
        if host["status"] == STATUS.DONE:
            host["latest_scan"][
                STATUS.DONE
            ] = current_time  # Update timestamp for reaching DONE status
            # check to see if owner should use a scheduler
            request = self.__db.requests.find_one(
                {"_id": host["owner"]}, {"scheduler": True}
            )
    ...
    def __update_hosts_next_scans(self, cursor, new_stage, new_status):
        ...
        for host in cursor:
            self.tally_update(
                host["owner"], host["stage"], host["status"], new_stage, new_status
            )
    ...
    def __process_open_ticket_age(self, results, open_as_of_date):
        ...
        if results:
            ...
            for (severity_name, severity_id) in [
                ...
                results_for_severity = df.loc[df["severity"] == severity_id][
                    "open_msec"
                ]
        ...
        if results:
            ...
            for (severity_name, severity_id) in [
                ...
                if results_for_severity.empty:
                    ...
                    age_median = long(np.median(results_for_severity))
                    age_max = long(np.max(results_for_severity))
                    closed_ticket_age[severity_name] = {
                        "median": age_median,
                        "max": age_max,
                    }
    ...
    def create_snapshot(
        ...
        if snapshot_doc.will_conflict():
            snapshot_doc[
                "end_time"
            ] = current_time  # Avoid conflicts by setting end_time to current time
        ...
        all_latest_snapshots = list(
            self.__db.SnapshotDoc.collection.find(
                {"latest": True},
                {"_id": 1, "parents": 1, "exclude_from_world_stats": 1},
            )
        )
        for snap in all_latest_snapshots:
            if (snap["_id"] not in snap["parents"]) or snap.get(
                "exclude_from_world_stats"
            ):
    ...
    def ignore_ticket(self, ip, port, source, source_id, reason):
        ticket = self.__db.TicketDoc.find_one(
            {
                "ip_int": long(ip),
                "port": port,
                "source": source,
        ...
        ticket.add_event(
            TICKET_EVENT.CHANGED,
            reason,
            delta={"from": False, "to": True, "key": "false_positive"},
        )
        ticket.save()
        return True
    ...
    def done_scanning(self, tally_time_after=None):
        ...
        for t in tallies:
            ...
            if tally_time_after and tally_time < tally_time_after:
                continue
            latest_snapshot = self.__db.SnapshotDoc.find_one(
                {"owner": owner, "latest": True}
            )
            request = self.__db.RequestDoc.get_by_owner(owner)
            owner_has_scheduler = request.get("scheduler") != None
            if owner_has_scheduler == False and (
                latest_snapshot == None or tally_time > latest_snapshot["last_change"]
            ):
    ...
    def should_commander_pause(self, apply_actions=True):
        docs = self.__db.SystemControlDoc.find(
            {"action": CONTROL_ACTION.PAUSE, "target": CONTROL_TARGET.COMMANDER}
        )
        if docs.count() == 0:
            return False
        if apply_actions:
